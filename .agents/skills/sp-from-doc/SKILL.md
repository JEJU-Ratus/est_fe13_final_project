---
name: sp-from-doc
description: "Read one existing Markdown specification from docs/specs together with the shared docs/specs/Header.md dependency, inspect their Figma frames, resolve every ambiguity without a question limit, and execute the complete speckit-specify workflow. Use when the user invokes $sp-from-doc with a page specification path or asks to convert an existing project specification into Spec Kit artifacts."
---
...
4. 입력 명세가 `docs/specs/Header.md`가 아니면 `docs/specs/Header.md`를 완전히 읽는다. 파일이 없으면
   사용자에게 알리고 중단한다. 그 외 참조 명세는 자동으로 따라가지 않는다.
5. 입력 명세와 Header 명세에서 Figma URL을 추출하되, Header는 현재 페이지에 적용되는 사용자 상태와
   펼침·접힘 상태의 프레임만 선택한다.
6. Figma 디자인을 조회하기 전에 설치된 `figma-design-to-code` 스킬을 완전히 읽고 그 지침을 따른다.
7. 선택한 각 Figma 프레임에 `get_design_context`를 호출해 상태별 구성과 시각적 차이를 확인한다.
8. 입력 명세와 적용 범위에 해당하는 Header 명세의 미정 사항 및 명세·Figma·기존 코드 충돌을 빠짐없이 수집한다.
9. 서로 다른 결정을 한 질문으로 합치지 말고 각 항목을 별도 질문으로 제시한다. 질문 개수를 제한하지 않는다.
10. 모든 질문에 대한 사용자 답변을 기다리고 결정 사항을 정리한다. 답변되지 않은 항목을 임의로 결정하지
   않는다.
11. 생성·수정할 파일, 기능 디렉터리, 브랜치 계획을 제시하고 사용자 승인을 기다린다.
12. `.agents/skills/speckit-specify/SKILL.md`를 완전히 읽는다.
13. 입력 명세, 적용되는 Header 요구사항, 사용자 답변, 확인한 Figma 정보를 기능 설명으로 사용해
    `speckit-specify`의 모든 절차를
    그대로 실행한다.
14. 생성 명세에 `docs/specs/Header.md`를 의존 명세로 기록하고 현재 페이지에 적용되는 Header 상태,
    디자인 프레임, 인수 조건과 기능 요구사항을 명시한다.
15. 품질 체크리스트 검증 중 새로운 미정 사항이 발견되면 산출물 작성을 멈추고 각 항목을 개별 질문한다.
16. 모든 답변을 명세와 체크리스트에 반영하고 `[NEEDS CLARIFICATION]`이 남지 않을 때까지 다시 검증한다.
17. 실행 후 훅을 확인한 다음 생성 경로, 체크리스트 결과, 다음 단계를 한국어로 보고한다.
...
- 페이지 기능의 `spec.md`에는 `docs/specs/Header.md` 의존 관계와 해당 페이지에 적용되는 Header 상태를
  반드시 기록한다.
- Header 요구사항 전체를 페이지 명세에 복제하지 않고, 현재 페이지에서 검증할 상태와 동작만 인수 조건과
  기능 요구사항으로 반영한다.
- 확인 필요 표시가 하나라도 남아 있으면 완료로 보고하지 않는다.

## 완료 조건

- 입력 명세, `docs/specs/Header.md`, 현재 페이지에 적용되는 모든 디자인 프레임을 확인했다.
- 생성 명세에 Header 의존 관계, 적용 상태, 디자인 참조와 검증 조건을 반영했다.
