...
7. 명세, Figma, 기존 코드의 충돌과 구현에 중대한 미정 사항을 사용자에게 보고한다.
8. 생성·수정할 파일, 기능 디렉터리, 브랜치 계획을 제시하고 사용자 승인을 기다린다.
9. `.agents/skills/speckit-specify/SKILL.md`를 완전히 읽는다.
10. 입력 명세의 전체 내용과 확인한 Figma 정보를 기능 설명으로 사용해 `speckit-specify`의 모든 절차를
    그대로 실행한다.
11. 품질 체크리스트를 검증하고 미정 사항이 있으면 `speckit-specify` 형식으로 최대 3개를 한 번에 질문한다.
12. 답변을 명세와 체크리스트에 반영하고 다시 검증한다.
13. 실행 후 훅을 확인한 다음 생성 경로, 체크리스트 결과, 다음 단계를 한국어로 보고한다.
...
- 확인 필요 표시가 남아 있으면 답변을 받기 전 완료로 보고하지 않는다.
