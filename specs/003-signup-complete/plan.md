...
**제약 사항**: Header와 사이트 레이아웃을 수정하지 않는다. 회원가입 완료 여부 판정, Loading, Suspense와 `loading.js`를 추가하지 않는다. 실제 인증 연동 단계의 로그인 사용자 접근에는 기존 CommonModal의 `alreadyLoggedIn` 모드를 사용한다. 승인된 프비 상하 애니메이션만 제공하고 이미지 원본 비율을 유지하며 새 자산이나 패키지를 만들지 않는다.
