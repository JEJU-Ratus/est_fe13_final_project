...
| `isLoginPersistent` | Boolean | `false` | 로그인 상태 유지 체크 여부다. UI에서만 관리하며 실제 세션 저장·복구는 추후 인증 연동 범위다. |
...
화면별 상태나 기능은 달라지지 않는다. 모든 화면에서 이메일 입력과 로그인 상태 유지 선택을 제공하며 배경, Input 그림자와 placeholder 스타일은 데스크톱 기준을 사용한다.
